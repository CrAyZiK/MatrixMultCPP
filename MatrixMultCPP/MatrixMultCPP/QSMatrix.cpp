...
template<typename T>
QSMatrix<T> QSMatrix<T>::Product(const QSMatrix<T>& mat) {
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            for (int k = 0; k < rows; k++) {
                result(i, j) += this->mat[i][k] * mat(k, j);
            }
        }
    }
    return result;
...
template<typename T>
void QSMatrix<T>::Randomize(T minVal, T maxVal) {
...
template<typename T>
T QSMatrix<T>::Spur() {
    auto size = std::min(get_rows(), get_cols());
    T spur = 0;
